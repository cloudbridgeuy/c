...
impl CompletionsApi {
    ...
    pub fn set_model(&mut self, model_name: String) -> Result<&mut Self, error::OpenAi> {
        log::debug!("Setting model to {}", model_name);

        let models = match ModelsApi::new(self.client.clone()).list() {
            Ok(models) => models,
            Err(err) => return Err(err),
        };

        if !models.iter().any(|m| m.id == model_name) {
            return Err(error::OpenAi::ModelNotFound { model_name });
        }

        self.model = model_name;

        log::debug!("Set model to {}", self.model);
