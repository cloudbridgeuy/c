...
use openai::chats::{ChatMessage, ChatsApi};
...
fn main() -> Result<(), openai::error::OpenAi> {
    ...
    match cli.command {
        ...
        Some(Commands::Chats { ref command }) => handle_chat(&cli, &command),
        None => {
            println!("{:#?}", cli);
            Ok(())
        }
...
fn handle_completions(
    ...
    match command {
        CompletionsCommands::Create {
            ...
            match cli.output {
                Output::Json => match serde_json::to_writer(std::io::stdout(), &completions) {
                    Ok(_) => {}
                    Err(e) => {
                        return Err(openai::error::OpenAi::SerializationError {
                            body: e.to_string(),
                        });
                    }
                },
                Output::Yaml => match serde_yaml::to_writer(std::io::stdout(), &completions) {
                    Ok(_) => {}
                    Err(e) => {
                        return Err(openai::error::OpenAi::SerializationError {
                            body: e.to_string(),
...
fn handle_chat(cli: &Cli, command: &ChatsCommands) -> Result<(), openai::error::OpenAi> {
    match command {
        ChatsCommands::Create {
            ...
            let mut api = match ChatsApi::new(get_api_key()) {
                Ok(api) => api,
                Err(e) => {
                    return Err(e);
                }
            ...
            let chats = api.create()?;

            match cli.output {
                Output::Json => match serde_json::to_writer(std::io::stdout(), &chats) {
                    Ok(_) => {}
                    Err(e) => {
                        return Err(openai::error::OpenAi::SerializationError {
                            body: e.to_string(),
                ...
                Output::Yaml => match serde_yaml::to_writer(std::io::stdout(), &chats) {
                    Ok(_) => {}
                    Err(e) => {
                        return Err(openai::error::OpenAi::SerializationError {
                            body: e.to_string(),
